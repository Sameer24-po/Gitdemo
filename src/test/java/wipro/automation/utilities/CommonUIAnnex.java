 ...
 *
 */
public class CommonUIAnnex {
	ElementController elementController;
	TestDataController testDataController;
	private ArrayList<String> taskList = new ArrayList<String>();

	public CommonUIAnnex(){		
		testDataController = new TestDataController();
		elementController = new ElementController(CommonUIAnnex.class.getSimpleName());
	}

	public CommonUIAnnex(String elementRepository) {
		testDataController = new TestDataController();
	...
	public String switchToWindowWithTitle(String windowTitle) throws Exception {
		...
		try {
			...
			try {
				parentWindow = BrowserDriverAnnex.getBrowserDriver().getWindowHandle();
			} catch(Exception exception) {}
			do {
				Set<String> availableWindows = BrowserDriverAnnex.getBrowserDriver().getWindowHandles();		
				for(String winHandle : availableWindows){
					try{
						BrowserDriverAnnex.getBrowserDriver().switchTo().window(winHandle);
						if(BrowserDriverAnnex.getBrowserDriver().getPageSource().contains(windowTitle)) {
							isWindowFind = true;
							return parentWindow;
						}
						else
	...
	public void takeScreenShot(String screenShotName) throws Exception {
		try {
			TakesScreenshot takesScreenshot=(TakesScreenshot)BrowserDriverAnnex.getBrowserDriver();
			File srcFile=takesScreenshot.getScreenshotAs(OutputType.FILE);
			FileUtils.copyFile(srcFile, new File(BasePath.mainpath + screenShotName + ".png"));
		} catch (Exception exception) { 
			throw exception;
		}
	}
	...
	public void quitAllBrowser() throws Exception {
		try {
			...
			BrowserDriverAnnex.getBrowserDriver().quit();
			elementController.implicitWait(1);
		}catch(Exception exception) {
	...
	public void enterTextIntoTextBox(String textBoxId, String text) throws Exception {
		try {
			if(text != null) {
				elementController.setWebelement(elementController.waitForWebElement(BrowserDriverAnnex.getBrowserDriver(), null, false, textBoxId));	
		...
		try {			
			if(button != null)
			{
				if(isDynamnicXpath == false)
					...
					elementController.waitForWebElement(BrowserDriverAnnex.getBrowserDriver(), null, false, button).click();
				else
					elementController.waitForWebElement(BrowserDriverAnnex.getBrowserDriver(), null, true, button).click();
			}
		}catch(Exception exception) {
	...
	public void clickOnButtonUsingJS(String button, boolean isDynamicXpath) throws Exception 
	{
		try {			
			if(button != null)
			{
				WebElement element = elementController.waitForWebElement(BrowserDriverAnnex.getBrowserDriver(), null, isDynamicXpath, button);
				JavascriptExecutor executor = (JavascriptExecutor)BrowserDriverAnnex.getBrowserDriver();
	...
	public void selectItemFromDropDownBox(String selectBoxID, String option)throws Exception{
		try{
			Select select = new Select(elementController.waitForWebElement(BrowserDriverAnnex.getBrowserDriver(), null, false, selectBoxID));
	...
	public void clickOnCheckBox(String checkBoxID, Boolean checkOrUncheck)throws Exception{
		try {
			if(null != checkOrUncheck){
				elementController.setWebelement(elementController.waitForWebElement(BrowserDriverAnnex.getBrowserDriver(), null, false, checkBoxID));
				if(checkOrUncheck){
